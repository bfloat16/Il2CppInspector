...
namespace Il2CppInspector
{
    public abstract partial class Il2CppBinary
    {
        ...
        private void TryPrepareMetadata(ulong codeRegistration, ulong metadataRegistration)
        {
            ...
            {
                throw new InvalidOperationException($"Could not analyze IL2CPP data: {ex.Message}", ex);
            }
        }
        ...
        private void PrepareMetadata(ulong codeRegistration, ulong metadataRegistration)
        {
            ...
            Console.WriteLine(
                "CodeRegistration struct found at 0x{0:X16} (file offset 0x{1:X8})",
                Image.Bits == 32 ? codeRegistration & 0xffff_ffff : codeRegistration,
                Image.MapVATR(codeRegistration)
            );
            Console.WriteLine(
                "MetadataRegistration struct found at 0x{0:X16} (file offset 0x{1:X8})",
                Image.Bits == 32 ? metadataRegistration & 0xffff_ffff : metadataRegistration,
             ...
             */
            if (
                (Metadata != null && Metadata.Types.Length != MetadataRegistration.TypeDefinitionsSizesCount)
                ...
                throw new NotSupportedException("The detected Il2CppCodeRegistration / Il2CppMetadataRegistration structs do not pass validation.");
            ...
            if (TypeReferences.Any(x => x.Type.IsTypeDefinitionEnum() && (uint)x.Data.KlassIndex >= (uint)Metadata.Types.Length))
            {
            ...
            var genericMethodTable = Image.ReadMappedVersionedObjectArray<Il2CppGenericMethodFunctionsDefinitions>(
