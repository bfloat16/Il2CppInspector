...
namespace Il2CppInspector
{
    ...
    {
        public Il2CppBinaryARM(IFileFormatStream stream, EventHandler<string> statusCallback = null)
        ...
        protected override (ulong, ulong) ConsiderCode(IFileFormatStream image, uint loc)
        {
            ...
            if (
                new byte[] { 0x00, 0x00, 0x8F, 0xE0, 0x01, 0x10, 0x8F, 0xE0 }.SequenceEqual(buff.Skip(0x10))
            ...
            if (regs.Count() == 5 && regs.TryGetValue(0, out _) && regs.TryGetValue(1, out r1) && regs.TryGetValue(2, out uint r2) && regs.TryGetValue(3, out uint r3))
            {
                if (r2 == 0 && r3 == 0)
                {
