...
namespace Il2CppInspector.Model
{
    ...
    public class AppModel : IEnumerable<CppType>
    {
        ...
        public AppModel(TypeModel model, bool makeDefaultBuild = true)
        {
            ...
            Exports = Image.GetExports()?.ToList() ?? [];
        ...
        public AppModel Build(UnityVersion unityVersion = null, CppCompilerType compiler = CppCompilerType.BinaryFormat, bool silent = false)
        {
            ...
            {
                Console.WriteLine($"Warning: selected version {UnityVersion} (metadata version {UnityHeaders.MetadataVersion})" + $" does not match metadata version {Image.Version}.");
            }
            ...
            var exports = TypeModel
                .Package.Binary.APIExports.Where(e => CppTypeCollection.TypedefAliases.ContainsKey(e.Key))
                .Select(e => new { VirtualAddress = e.Value, FnPtr = CppTypeCollection.TypedefAliases[e.Key] });
            ...
            var unusedConcreteTypes = unusedTypes.Where(t => !t.IsGenericType && !t.IsGenericParameter && !t.IsByRef && !t.IsPointer && !t.IsArray && !t.IsAbstract && t.Name != "<Module>");
