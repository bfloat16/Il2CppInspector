...
namespace Il2CppInspector.Model
{
    ...
    public class AddressMap : IDictionary<ulong, object>
    {
        ...
        // - Export                      (for exports)
        // - Symbol                      (for symbols)
        public SortedDictionary<ulong, object> Items { get; } = [];
        ...
        {
            get => ((IDictionary<ulong, object>)Items)[key];
            set => ((IDictionary<ulong, object>)Items)[key] = value;
        }
        ...
        private void build()
        {
            ...
            foreach (var type in Model.Types.Values)
            {
                if (type.TypeClassAddress != 0xffffffff_ffffffff)
                    Add(type.TypeClassAddress, new AppTypeReference { Field = new CppField($"{type.Name}__TypeInfo", classPtrType), Type = type });
