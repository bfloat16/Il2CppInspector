...
namespace Il2CppInspector.Reflection
{
    public static class Extensions
    {
        ...
        )
        {
            ...
            foreach (var cad in attributes)
            {
                ...
                else
                {
                    ...
                    var commentStart =
                        mustCompile && !parameterlessConstructor
                            ? inline
                                ? "/* "
                                : "// "
                            : "";
        ...
        public static string ToAddressString(this ulong address) => address <= 0xffff_ffff ? string.Format($"0x{(uint)address:X8}") : string.Format($"0x{address:X16}");
        ...
        private static Dictionary<char, string> escapeChars = new()
        {
            ['\''] = @"\'",
            ['"'] = @"\""",
            ['\\'] = @"\\",
