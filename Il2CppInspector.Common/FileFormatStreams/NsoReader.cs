...
internal class ByteArrayBackingBuffer : IDisposable
{
    ...
    public T ReadObject<T>(ulong address, in StructVersion version)
        where T : unmanaged, IReadable
    {
        Debug.Assert(Initialized);

        return IsLittleEndian
            ? Reader.LittleEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadVersionedObject<T>(in version)
            : Reader.BigEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadVersionedObject<T>(in version);
    }

    public ImmutableArray<T> ReadObjectArray<T>(ulong address, long count, in StructVersion version)
        where T : unmanaged, IReadable
    {
        if (count == 0)
            return [];
        ...
        return IsLittleEndian
            ? Reader.LittleEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadVersionedObjectArray<T>(count, in version)
            : Reader.BigEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadVersionedObjectArray<T>(count, in version);
    }

    public T ReadPrimitive<T>(ulong address)
        where T : unmanaged
    {
        Debug.Assert(Initialized);

        return IsLittleEndian
            ? Reader.LittleEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadPrimitive<T>()
            : Reader.BigEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadPrimitive<T>();
    }

    public ImmutableArray<T> ReadPrimitiveArray<T>(ulong address, long count)
        where T : unmanaged
    {
        Debug.Assert(Initialized);

        return IsLittleEndian
            ? Reader.LittleEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadPrimitiveArray<T>(count)
            : Reader.BigEndian(Data, TranslateVaToRva(address), new ReaderConfig(Is32Bit)).ReadPrimitiveArray<T>(count);
    }
...
public class NsoReader : FileFormatStream<NsoReader>
{
    ...
    {
        var reader = new Reader<BinaryObjectStreamReader>(this) { Offset = checked((int)header.FileOffset) };
    ...
    {
        if (!_dynamicEntries.TryGetValue(DynamicTag.DT_SYMTAB, out var symtabAddress) || !_dynamicEntries.TryGetValue(DynamicTag.DT_SYMENT, out var symtabEntrySize))
            return;
        ...
        if (_dynamicEntries.TryGetValue(DynamicTag.DT_JMPREL, out var jmprelAddress))
        {
            ...
            ApplyRelocationsImpl(type == DynamicTag.DT_REL ? ParseRelSection(jmprelAddress, size) : ParseRelaSection(jmprelAddress, size));
        ...
        List<(ulong, ulong, long?)> ParseRelSection(ulong rva, ulong size)
        {
            ...
            return [.. _mappedExecutable.ReadObjectArray<RelEntry>(ImageBase + rva, checked((int)entryCount), default).Select<RelEntry, (ulong, ulong, long?)>(x => (x.Offset, x.Info, null))];
        }
        ...
        List<(ulong, ulong, long?)> ParseRelaSection(ulong rva, ulong size)
        {
            ...
            return [.. _mappedExecutable.ReadObjectArray<RelaEntry>(ImageBase + rva, checked((int)entryCount), default).Select<RelaEntry, (ulong, ulong, long?)>(x => (x.Offset, x.Info, x.Addend))];
        }
        ...
        void ApplyRelrRelocations(ulong address, ulong size)
        {
            ...
            void ApplyRelrRelocationsImpl<T>()
                where T : unmanaged, IUnsignedNumber<T>, IBinaryNumber<T>
            {
    ...
    public override ulong MapFileOffsetToVA(uint offset) => ImageBase + offset;
}
