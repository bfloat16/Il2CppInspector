...
namespace Il2CppInspector
{
        ...
        where TReader : FileFormatStream<TReader>
    {
        ...
        public override string Arch =>
            (Elf)ElfHeader.e_machine switch
            {
                Elf.EM_386 => "x86",
                Elf.EM_ARM => "ARM",
                Elf.EM_X86_64 => "x64",
        ...
        public Dictionary<string, elf_shdr<TWord>> SectionByName = [];
        ...
        private List<(uint Start, uint End)> reverseMapExclusions = [];
        ...
        private readonly TConvert conv = new();
        ...
        protected class ElfReloc
        {
            ...
            public ElfReloc(elf_rela<TWord> rela, TWord symbolTable)
                : this(new elf_rel<TWord> { r_info = rela.r_info, r_offset = rela.r_offset }, symbolTable) => Addend = rela.r_addend;
        ...
        protected override bool Init()
        {
            ...
            {
                var shtShouldBeOrdered = SHT.Where(s => ((Elf)conv.Int(s.sh_flags) & Elf.SHF_ALLOC) == Elf.SHF_ALLOC)
                    .OrderBy(s => s.sh_addr)
            ...
            foreach (var rel in rels)
            {
                ...
                try
                {
                    ...
                    symValue = ReadMappedObject<TSym>(conv.ULong(rel.SymbolTable) + conv.ULong(rel.SymbolIndex) * relsz).st_value; // S
                }
                catch (InvalidOperationException)
                {
                    try
                    {
                        symValue = ReadObject<TSym>(conv.Long(rel.SymbolTable) + conv.Long(rel.SymbolIndex) * relsz).st_value; // S
                    }
                    catch (InvalidOperationException)
                    {
                        continue;
        ...
        private void processSymbols()
        {
                ...
                if (GetDynamicEntry(Elf.DT_SYMTAB) is elf_dynamic<TWord> DT_SYMTAB)
                {
                    if (GetDynamicEntry(Elf.DT_STRSZ) is elf_dynamic<TWord> DT_STRSZ)
                    {
                        ...
                        if (orderedTable.Count != 0)
                        {
                            ...
                            pTables.Add((conv.FromUInt(MapVATR(conv.ULong(DT_SYMTAB.d_un))), conv.Div(conv.Sub(end, DT_SYMTAB.d_un), Sizeof(typeof(TSym))), DT_STRTAB.d_un, DT_STRSZ.d_un));
                        }
                    }
                }
            }
            ...
            {
                if (alreadyProcessed.Any(x => conv.ULong(x.offset) == conv.ULong(pTab.offset)))
                    continue;
                ...
                foreach (var symbol in symbol_table)
                {
                    ...
                    var type =
                        symbol.type == Elf.STT_FUNC ? SymbolType.Function
                    ...
                    var symbolItem = new Symbol
                    {
                        Name = name,
                        Type = type,
                        VirtualAddress = conv.ULong(symbol.st_value),
                    };
        ...
        public override uint[] GetFunctionTable()
        {
            ...
            if (GetDynamicEntry(Elf.DT_INIT) != null)
                init_array = init_array.Concat(conv.UIntArray([GetDynamicEntry(Elf.DT_INIT).d_un])).ToArray();
