...
public interface IIndexType<T>
    where T : IIndexType<T>, allows ref struct
{
    ...
    public static int IndexSize(in StructVersion version = default, in ReaderConfig config = default) => GetSizeFromTag(version);

    public static int ReadIndex<TReader>(ref Reader<TReader> reader, in StructVersion version = default)
