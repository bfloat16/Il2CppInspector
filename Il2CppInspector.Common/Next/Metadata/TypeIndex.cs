...
public struct TypeIndex(int value) : IIndexType<TypeIndex>, IReadable, IEquatable<TypeIndex>
{
    ...
    public static int Size(in StructVersion version = default, in ReaderConfig config = default) => IIndexType<TypeIndex>.IndexSize(version, config);
    ...
    public static bool operator ==(TypeIndex left, TypeIndex right) => left._value == right._value;

    public static bool operator !=(TypeIndex left, TypeIndex right) => !(left == right);

    public override readonly bool Equals(object obj) => obj is TypeIndex other && Equals(other);

    public readonly bool Equals(TypeIndex other) => this == other;
