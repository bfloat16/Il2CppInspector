...
public struct MethodIndex(int value) : IIndexType<MethodIndex>, IReadable, IEquatable<MethodIndex>
{
    ...
    public static int Size(in StructVersion version = default, in ReaderConfig config = default) => IIndexType<MethodIndex>.IndexSize(version, config);
    ...
    public static bool operator ==(MethodIndex left, MethodIndex right) => left._value == right._value;

    public static bool operator !=(MethodIndex left, MethodIndex right) => !(left == right);

    public override readonly bool Equals(object obj) => obj is MethodIndex other && Equals(other);

    public readonly bool Equals(MethodIndex other) => this == other;
