...
public record struct Il2CppType : IReadable
{
        ...
        where TReader : IReader, allows ref struct
    {
        ...
        if (MetadataVersions.V272 > version)
        {
            ...
            if (numModifiers > 31)
                throw new InvalidOperationException("Versions pre-27.2 with a type having more than 31 modifiers are not supported yet");

            // Else, we do some bit-juggling to convert the old value into the new format:
            Value =
                (Value & 0xFFFFFF)
                | // Attributes + Type
                (((Value >> 24) & 0b111111) << 24)
                | // 5 Bits for the modifiers
                (((Value >> 30) & 1) << 29)
                | // Shifted ByRef
                (((Value >> 31) & 1) << 30)
                | // Shifted Pinned
    ...
    public static Il2CppType FromTypeEnum(Il2CppTypeEnum type) => new() { Value = (uint)type << 16 };
}
