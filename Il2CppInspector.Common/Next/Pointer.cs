...
public struct Pointer<T>(ulong value = 0) : IReadable, IEquatable<Pointer<T>>
    where T : struct, IReadable
{
    [NativeInteger]
    ...
    public static bool operator ==(Pointer<T> left, Pointer<T> right) => left._value == right._value;

    public static bool operator !=(Pointer<T> left, Pointer<T> right) => !(left == right);

    public override readonly bool Equals(object? obj) => obj is Pointer<T> other && Equals(other);

    public readonly bool Equals(Pointer<T> other) => this == other;
