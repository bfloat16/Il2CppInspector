...
public ref struct NameTranslationParserContext
{
    ...
    private NameTranslationInfo Parse()
    {
        ...
        foreach (var line in _nameTranslationLines)
        {
            ...
            if (nameTranslationMap.TryGetValue(obfuscatedName, out var alreadySeenName))
            {
                if (alreadySeenName != deobfuscatedName)
                    throw new InvalidDataException($"Name translation collision detected for name {obfuscatedName}: {alreadySeenName} vs. {deobfuscatedName}");
            }
            else
            {
        ...
        return new NameTranslationInfo(nameTranslationMap.ToFrozenDictionary(), namespaceTranslationMap.ToFrozenDictionary());
    }
    ...
    private static (string, string?) ParseFullClassName(string fullName)
    {
        ...
        var ns = lastNamespaceIdx == -1 ? null : fullName[..lastNamespaceIdx];
