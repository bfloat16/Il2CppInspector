...
public struct PrimitivePointer<T>(ulong value = 0) : IReadable, IEquatable<PrimitivePointer<T>>
    where T : unmanaged
{
    [NativeInteger]
    ...
    public static bool operator ==(PrimitivePointer<T> left, PrimitivePointer<T> right) => left._value == right._value;

    public static bool operator !=(PrimitivePointer<T> left, PrimitivePointer<T> right) => !(left == right);

    public override readonly bool Equals(object? obj) => obj is PrimitivePointer<T> other && Equals(other);

    public readonly bool Equals(PrimitivePointer<T> other) => this == other;

    public override readonly int GetHashCode() => HashCode.Combine(_value);

    public override readonly string ToString() => $"0x{_value:X} <{typeof(T).Name}>";
