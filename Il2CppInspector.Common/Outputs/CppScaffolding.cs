...
namespace Il2CppInspector.Outputs
{
    public partial class CppScaffolding(AppModel model, bool useBetterArraySize = false)
    {
         ...
         */
        private readonly bool _useBetterArraySize = model.UnityVersion.CompareTo("2017.2.1") >= 0 && model.Package.BinaryImage.Bits == 64 && useBetterArraySize;
        ...
        public void Write(string projectPath)
        {
            ...
            using (_writer)
            {
                ...
                foreach (var method in _model.Methods.Values)
                {
                    if (method.HasCompiledCode)
                    {
                        ...
                        writeCode(
                            $"DO_APP_FUNC(0x{method.MethodCodeAddress - _model.Package.BinaryImage.ImageBase:X8}, {method.CppFnPtrType.ReturnType.Name}, "
            ...
            void WriteIfNotExists(string path, string contents)
            {
                if (!File.Exists(path))
            ...
            WriteIfNotExists(Path.Combine(projectPath, projectFile), Resources.CppProjTemplate.Replace("%PROJECTGUID%", projectGuid.ToString()));
            ...
            var filters = Resources.CppProjFilters.Replace("%GUID1%", guid1.ToString()).Replace("%GUID2%", guid2.ToString()).Replace("%GUID3%", guid3.ToString());
            ...
            var sln = Resources
                .CppSlnTemplate.Replace("%PROJECTGUID%", projectGuid.ToString())
