...
namespace Il2CppInspector.Outputs
{
    public class CSharpCodeStubs
    {
        ...
        private HashSet<CustomAttributeData> usedAssemblyAttributes = [];
        private readonly object usedAssemblyAttributesLock = new();
        ...
        public void WriteFilesByNamespace<TKey>(string outPath, Func<TypeInfo, TKey> orderBy, bool flattenHierarchy)
        {
            usedAssemblyAttributes.Clear();
            Parallel.ForEach(
                model.Assemblies.SelectMany(x => x.DefinedTypes).GroupBy(t => t.Namespace),
                ns =>
                {
                    var relPath = !string.IsNullOrEmpty(ns.Key) ? ns.Key : "global";
                    writeFile(Path.Combine(outPath, (flattenHierarchy ? relPath : Path.Combine(relPath.Split('.'))) + ".cs"), ns.OrderBy(orderBy));
                }
            );
        }
        ...
        public void WriteFilesByAssembly<TKey>(string outPath, Func<TypeInfo, TKey> orderBy, bool separateAttributes)
        {
            ...
            Parallel.ForEach(
                ...
                asm =>
                {
                    ...
                    if (
                        writeFile(
                            Path.Combine(outPath, Path.GetFileNameWithoutExtension(asm.ShortName) + ".cs"),
                            asm.DefinedTypes.OrderBy(t => t.Namespace).ThenBy(orderBy),
                            outputAssemblyAttributes: !separateAttributes
                        ) && separateAttributes
                    )
                    {
        ...
        public void WriteFilesByClass(string outPath, bool flattenHierarchy)
        {
            usedAssemblyAttributes.Clear();
            Parallel.ForEach(
                model.Assemblies.SelectMany(x => x.DefinedTypes),
                type =>
                {
        ...
        public HashSet<Assembly> WriteFilesByClassTree(string outPath, bool separateAttributes)
        {
            ...
            Parallel.ForEach(
                ...
                {
                    lock (usedAssemblies)
                        usedAssemblies.UnionWith(usedPartition);
                }
            );
        ...
        public void WriteSolution(string outPath, string unityPath, string unityAssembliesPath)
        {
            ...
            foreach (var asm in assemblies)
            {
                ...
                var def = Resources.SlnProjectDefinition.Replace("%PROJECTGUID%", guid.ToString()).Replace("%PROJECTNAME%", name).Replace("%CSPROJRELATIVEPATH%", csProjFile);

                slnProjectDefs.Append(def);

                var config = Resources.SlnProjectConfiguration.Replace("%PROJECTGUID%", guid.ToString());

                slnProjectConfigs.Append(config);

                // Determine all the assemblies on which this assembly depends
                var dependencyTypes = asm.DefinedTypes.SelectMany(t => t.GetAllTypeReferences()).Union(asm.CustomAttributes.SelectMany(a => a.AttributeType.GetAllTypeReferences())).Distinct();
                var dependencyAssemblies = dependencyTypes.Select(t => t.Assembly).Distinct().Except(new[] { asm });
                ...
                var referenceXml = string.Concat(
                    dependencyAssemblies.Select(a => $@"    <ProjectReference Include=""..\{a.ShortName.Replace(".dll", "")}\{a.ShortName.Replace(".dll", "")}.csproj""/>" + "\n")
                );

                // Create a .csproj file using the project Guid
                var csProj = Resources
                    .CsProjTemplate.Replace("%PROJECTGUID%", guid.ToString())
            ...
            var sln = Resources.CsSlnTemplate.Replace("%PROJECTDEFINITIONS%", slnProjectDefs.ToString()).Replace("%PROJECTCONFIGURATIONS%", slnProjectConfigs.ToString());
        ...
        private bool writeFile(string outFile, IEnumerable<TypeInfo> types, bool useNamespaceSyntax = true, bool outputAssemblyAttributes = true)
        {
            ...
            do
            {
                try
                {
                    using StreamWriter writer = new(new FileStream(outFile, FileMode.Create), Encoding.UTF8);
...
"
        ...
        private string generateAssemblyInfo(IEnumerable<Reflection.Assembly> assemblies, IEnumerable<string> namespaces = null, bool outputAssemblyAttributes = true)
        {
            ...
            foreach (var asm in assemblies)
            {
                ...
                if (outputAssemblyAttributes)
                    ...
                    {
                        text.Append(
                            asm.CustomAttributes.Where(a => a.AttributeType.FullName != ExtAttribute)
                                .Except(usedAssemblyAttributes ?? [])
                                .OrderBy(a => a.AttributeType.Name)
                                .ToString(
                                    new Scope { Current = null, Namespaces = namespaces ?? new List<string>() },
                                    attributePrefix: "assembly: ",
                                    emitPointer: !SuppressMetadata,
        ...
        private StringBuilder generateType(TypeInfo type, IEnumerable<string> namespaces, string prefix = "")
        {
            ...
            var scope = new Scope { Current = type, Namespaces = namespaces };
            ...
            if (!type.IsEnum)
            {
                foreach (var field in type.DeclaredFields)
                {
                    ...
                    sb.Append(
                        field
                            .CustomAttributes.Where(a => a.AttributeType.FullName != FBAttribute)
                            .OrderBy(a => a.AttributeType.Name)
            ...
            {
                // Attributes
                sb.Append(prop.CustomAttributes.OrderBy(a => a.AttributeType.Name).ToString(scope, prefix + "\t", emitPointer: !SuppressMetadata, mustCompile: MustCompile));
                ...
                else
                {
                    ...
                    sb.Append(
                        "this["
                            + string.Join(", ", primary.DeclaredParameters.SkipLast(getAccess >= setAccess ? 0 : 1).Select(p => p.GetParameterString(scope, !SuppressMetadata, MustCompile)))
                            + "] { "
                    );
                    getBody = " => default;";
                    setBody = " {}";
                ...
                sb.Append(
                    (
                        prop.CanRead
                            ? prop.GetMethod.CustomAttributes.Where(a => !MustCompile || a.AttributeType.FullName != CGAttribute)
                                .ToString(scope, inline: true, emitPointer: !SuppressMetadata, mustCompile: MustCompile)
                                + (getAccess < setAccess ? prop.GetMethod.GetAccessModifierString() : "")
                                + $"get{getBody} "
                            : ""
                    )
                        ...
                        + (
                            prop.CanWrite
                                ? prop.SetMethod.CustomAttributes.Where(a => !MustCompile || a.AttributeType.FullName != CGAttribute)
                                    .ToString(scope, inline: true, emitPointer: !SuppressMetadata, mustCompile: MustCompile)
                                    + (setAccess < getAccess ? prop.SetMethod.GetAccessModifierString() : "")
                                    + $"set{setBody} "
                                : ""
                        )
                        + "}"
                ...
                if (!SuppressMetadata)
                {
                    if ((prop.CanRead && prop.GetMethod.VirtualAddress != null) || (prop.CanWrite && prop.SetMethod.VirtualAddress != null))
                        sb.Append(" // ");
                    sb.Append(
                        (prop.CanRead && prop.GetMethod.VirtualAddress != null ? prop.GetMethod.VirtualAddress.ToAddressString() + " " : "")
            ...
            {
                // Attributes
                sb.Append(evt.CustomAttributes.OrderBy(a => a.AttributeType.Name).ToString(scope, prefix + "\t", emitPointer: !SuppressMetadata, mustCompile: MustCompile));
                ...
                if (!MustCompile)
                {
                    ...
                    if (evt.AddMethod != null)
                        m.Add("add", evt.AddMethod.VirtualAddress);
                    if (evt.RemoveMethod != null)
                        m.Add("remove", evt.RemoveMethod.VirtualAddress);
                    if (evt.RaiseMethod != null)
                        m.Add("raise", evt.RaiseMethod.VirtualAddress);
            ...
            // Nested types
            codeBlocks.Add("Nested types", new StringBuilder().AppendJoin("\n", type.DeclaredNestedTypes.Select(n => generateType(n, namespaces, prefix + "\t")).Where(c => c.Length > 0)));
            ...
            if (MustCompile && !type.IsInterface && !(type.IsAbstract && type.IsSealed) && !type.IsValueType && type.DeclaredConstructors.All(c => c.IsStatic || c.DeclaredParameters.Any()))
            ...
            {
                // Attributes
                sb.Append(method.CustomAttributes.OrderBy(a => a.AttributeType.Name).ToString(scope, prefix + "\t", emitPointer: !SuppressMetadata, mustCompile: MustCompile));
            ...
            codeBlocks.Add("Methods", methods.Select(m => generateMethod(m, scope, prefix)).Aggregate(new StringBuilder(), (r, i) => r.Append(i)));

            usedMethods.AddRange(methods);

            // Extension methods
            codeBlocks.Add("Extension methods", type.DeclaredMethods.Except(usedMethods).Select(m => generateMethod(m, scope, prefix)).Aggregate(new StringBuilder(), (r, i) => r.Append(i)));
            ...
            sb.Append(
                type.CustomAttributes.Where(a => (a.AttributeType.FullName != DMAttribute || !hasIndexer) && a.AttributeType.FullName != ExtAttribute)
                    .OrderBy(a => a.AttributeType.Name)
                    .ToString(scope, prefix, emitPointer: !SuppressMetadata, mustCompile: MustCompile)
            );
            ...
            if (type.IsEnum)
            {
                sb.AppendJoin(",\n", type.GetEnumNames().Zip(type.GetEnumValues().OfType<object>(), (k, v) => new { k, v }).OrderBy(x => x.v).Select(x => $"{prefix}\t{x.k} = {x.v}"));
                sb.Append("\n");
            }
            // Type definition
            else
        ...
        private StringBuilder generateMethod(MethodInfo method, Scope scope, string prefix)
        {
            ...
            writer.Append(
                method
                    .CustomAttributes.Where(a => a.AttributeType.FullName != ExtAttribute && a.AttributeType.FullName != AsyncAttribute)
                    .OrderBy(a => a.AttributeType.Name)
                    .ToString(scope, prefix + "\t", emitPointer: !SuppressMetadata, mustCompile: MustCompile)
            );
