...
public class CppDeclarationGenerator
{
    ...
    private readonly Dictionary<TypeInfo, TypeInfo> _concreteImplementations = [];
    ...
    public List<(
        TypeInfo ilType,
        CppComplexType valueType,
        CppComplexType referenceType,
        CppComplexType fieldsType,
        CppComplexType vtableType,
        CppComplexType staticsType
    )> GenerateRemainingTypeDeclarations()
    {
        var decl = GenerateVisitedFieldStructs().Select(s => (s.ilType, s.valueType, s.referenceType, s.fieldsType, (CppComplexType)null, (CppComplexType)null)).ToList();
    ...
    */
    ...
    private void InitializeNaming()
    {
        ...
        TypeNamer = TypeNamespace.MakeNamer<TypeInfo>(
            (ti) =>
            {
                if (ti.IsArray)
