...
namespace Il2CppInspector.Cpp
{
    ...
    public class CppTypeCollection : IEnumerable<CppType>
    {
        ...
        public CppType this[string s] =>
            Types.ContainsKey(s) ? Types[s]
            : TypedefAliases.ContainsKey(s) ? TypedefAliases[s].AsAlias(s)
            : null;
        ...
        public void AddFromDeclarationText(string text)
        {
            ...
            while ((rawLine = line = lines.ReadLine()) != null)
            {
                ...
                if (externDecl.Success)
                {
                    ...
                    switch (complexType)
                    {
                        case ComplexValueType.Struct:
                            Struct(declType);
                            break;
                        case ComplexValueType.Union:
                            Union(declType);
                            break;
                    }
        ...
        public int AddField(CppComplexType declaringType, string fieldName, string typeName, bool isConst = false) => declaringType.AddField(fieldName, GetType(typeName), isConst: isConst);
        ...
        public CppComplexType Struct(string name = "", int alignmentBytes = 0)
        {
            ...
            var type = new CppComplexType(ComplexValueType.Struct)
            {
                Name = name,
                Group = currentGroup,
        ...
        public CppComplexType Union(string name = "", int alignmentBytes = 0)
        {
            ...
            var type = new CppComplexType(ComplexValueType.Union)
            {
                Name = name,
                Group = currentGroup,
                AlignmentBytes = alignmentBytes,
