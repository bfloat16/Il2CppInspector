...
namespace Il2CppInspector.Cpp
{
    ...
    public class CppType
    {
        ...
        public CppPointerType AsPointer(int WordSize) => new(WordSize, this);

        // Generate array of this type
        public CppArrayType AsArray(int Length) => new(this, Length);
    ...
    public class CppPointerType : CppType
    {
        ...
        public CppPointerType(int WordSize, CppType elementType)
            : base(null, WordSize) => ElementType = elementType;
    ...
    public class CppArrayType : CppType
    {
        ...
        public CppArrayType(CppType elementType, int length)
            : base()
        {
        ...
        // Return the type as a field
        public override string ToFieldString(string fieldName, string format = "") => ElementType.ToFieldString(fieldName, format) + "[" + Length + "]";
    ...
    public class CppFnPtrType : CppType
    {
        ...
        public static CppFnPtrType FromSignature(CppTypeCollection types, string text)
        {
            ...
            foreach (var argument in arguments)
            {
                ...
                {
                    name =
                        argument.IndexOf("*") != -1 ? argument.Substring(argument.LastIndexOf("*") + 1).Trim()
        ...
        public override string ToFieldString(string name, string format = "") =>
        ...
        public string ToSignatureString() =>
            $"{ReturnType.Name} {Name}(" + string.Join(", ", Arguments.Select(a => a.Type is CppFnPtrType fn ? fn.ToFieldString(a.Name) : a.Type.Name + (a.Name.Length > 0 ? " " + a.Name : ""))) + ")";
    }
    ...
    public class CppComplexType : CppType, IEnumerable<CppField>
    {
            ...
            private SortedDictionary<int, List<CppField>> getFlattenedFields(CppComplexType t)
            {
                ...
                foreach (var field in t.Fields.Values.SelectMany(f => f))
                {
                    ...
                    if (type is CppComplexType ct)
                    {
                        ...
                        var fields = ct
                            .Flattened.Fields.Select(kl => new
                            {
                    ...
                    else
                    {
                        if (flattened.ContainsKey(field.Offset))
                            ...
                            flattened.Add(field.Offset, [field]);
                    }
                }
                return flattened;
            }
        ...
        public int AddField(string name, CppType type, int alignmentBytes = 0, int bitfield = 0, bool isConst = false) => AddField(new CppField(name, type, bitfield, isConst), alignmentBytes);

        // Return the type as a field
        public override string ToFieldString(string fieldName, string format = "") => (ComplexValueType == ComplexValueType.Struct ? "struct " : "union ") + Name + " " + fieldName;
    ...
    {
        public CppForwardDefinitionType(string name)
            : base(name) { }
