...
public class BinaryObjectStream : MemoryStream
{
    ...
    public BinaryObjectStream(byte[] bytes, Endianness endianness = Endianness.Little, bool leaveOpen = false)
        : base(bytes)
    {
    ...
    public T ReadObject<T>(long addr)
        where T : new() => Reader.ReadObject<T>(addr);

    public T ReadObject<T>()
        where T : new() => Reader.ReadObject<T>();

    public T[] ReadArray<T>(long addr, int count)
        where T : new() => Reader.ReadArray<T>(addr, count);

    public T[] ReadArray<T>(int count)
        where T : new() => Reader.ReadArray<T>(count);
