...
public class BinaryObjectReader : BinaryReader
{
    ...
    private Dictionary<string, MethodInfo> readObjectGenericCache = [];

    // VersionAttribute cache to dramatically speed up repeated calls to ReadObject<T> with the same T
    private Dictionary<Type, Dictionary<FieldInfo, List<(double Min, double Max)>>> readObjectVersionCache = [];

    // Thread synchronization objects (for thread safety)
    private object readLock = new();

    // Initialization
    public BinaryObjectReader(Stream stream, Endianness endianness = Endianness.Little, bool leaveOpen = false)
        : base(stream, Encoding.Default, leaveOpen)
    {
        Endianness = endianness;

        readMethodCache = typeof(BinaryObjectReader)
            .GetMethods()
    ...
    public Dictionary<string, Type> PrimitiveMappings { get; } = [];
    ...
    public Dictionary<Type, Type> ObjectMappings { get; } = [];
    ...
    public override byte[] ReadBytes(int count)
    {
        ...
        return (Endianness == Endianness.Little ? bytes : bytes.AsEnumerable().Reverse().ToArray());
    }
    ...
    public int ReadCompressedInt32(long addr)
    {
        ...
        return signFlag == 1 ? -(int)(val + 1) : (int)val;
    }
    ...
    public T ReadObject<T>(long addr)
        where T : new()
    {
        lock (readLock)
        {
    ...
    private T MapObject<T>(object from)
        where T : new()
    {
        var t = new T();
        ...
        where T : new()
    {
        ...
        foreach (var (i, versions) in readObjectVersionCache[type])
        {
            ...
            {
                var attr = i.GetCustomAttribute<ArrayLengthAttribute>(false) ?? throw new InvalidOperationException("Array field " + i.Name + " must have ArrayLength attribute");

                int lengthPrimitive;

                if (attr.FieldName != null)
                {
                    var field = type.GetField(attr.FieldName) ?? throw new ArgumentException("Array field " + i.Name + " has invalid FieldName in ArrayLength attribute");
                ...
                var us = GetType().GetMethod("ReadArray", [typeof(int)]);
                var mi2 = us.MakeGenericMethod(i.FieldType.GetElementType());
                i.SetValue(t, mi2.Invoke(this, [lengthPrimitive]));
            }
            // Primitive type
    ...
    public T[] ReadArray<T>(long addr, int count)
        where T : new()
    {
        lock (readLock)
        {
