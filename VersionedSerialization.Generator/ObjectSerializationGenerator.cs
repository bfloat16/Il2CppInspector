...
public sealed class ObjectSerializationGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var valueProvider = context
            .SyntaxProvider.ForAttributeWithMetadataName(
                Constants.VersionedStructAttribute,
    ...
    private static void GenerateReadMethod(CodeGenerator generator, ObjectSerializationInfo info)
    {
        ...
        foreach (var property in info.Properties)
        {
            ...
            generator.EnterScope();
            generator.AppendLine(property.Type.NeedsAssignmentToMember() ? $"this.{property.Name} = {property.ReadMethod}" : property.ReadMethod);
    ...
    private static string GetVersionIdentifier(StructVersion version) => $"V{version.Major}_{version.Minor}{(version.Tag == null ? "" : $"_{version.Tag}")}";

    private static void GenerateVersionCondition(ImmutableEquatableArray<VersionCondition> conditions, CodeGenerator generator)
    {
        ...
        for (var i = 0; i < conditions.Length; i++)
        {
            ...
            if (condition.IncludingTag != null)
                generator.AppendLine(condition.IncludingTag == "" ? "&& version.Tag == null" : $"&& version.Tag == \"{condition.IncludingTag}\"");

            if (condition.ExcludingTag != null)
                generator.AppendLine(condition.ExcludingTag == "" ? "&& version.Tag != null" : $"&& version.Tag != \"{condition.ExcludingTag}\"");
    ...
    private static ObjectSerializationInfo ParseSerializationInfo(TypeDeclarationSyntax contextClass, SemanticModel model, Compilation compilation, CancellationToken cancellationToken)
    {
        ...
        {
            if (member.IsStatic || member is IFieldSymbol { AssociatedSymbol: not null } || member is IPropertySymbol { SetMethod: null })
                continue;
            ...
            foreach (var attribute in member.GetAttributes())
            {
                if (SymbolEqualityComparer.Default.Equals(attribute.AttributeClass, versionConditionAttribute))
                {
                    ...
                    var condition = new VersionCondition(lessThan, moreThan, equalTo, lessThanOrEqual, moreThanOrEqual, includingTag, excludingTag);
                ...
                {
                    var nativeIntegerType = typeInfo.Type.IsUnsignedType() ? PropertyType.UNativeInteger : PropertyType.NativeInteger;

                    var complexTypeName = typeInfo.ComplexTypeName == "" ? typeInfo.Type.GetTypeName() : typeInfo.ComplexTypeName;
            ...
            if (readMethod == null)
            {
                ...
                {
                    readMethod = typeInfo.Type.IsSeperateMethod() ? $"reader.Read{typeInfo.Type.GetTypeName()}();" : $"reader.ReadPrimitive<{typeInfo.Type.GetTypeName()}>();";
            ...
            sizeExpression ??= typeInfo.Type switch
            {
                ...
                PropertyType.NativeInteger or PropertyType.UNativeInteger => "config.Is32Bit ? sizeof(uint) : sizeof(ulong)",
                PropertyType.Bytes => $"Unsafe.SizeOf<{typeInfo.ComplexTypeName}>()",
                _ => $"sizeof({typeInfo.Type.GetTypeName()})",
            };

            properties.Add(new PropertySerializationInfo(member.Name, readMethod, sizeExpression, typeInfo.Type, versionConditions.ToImmutableEquatableArray()));
        }
        ...
        if (classSymbol.BaseType != null)
        {
            ...
            if (!SymbolEqualityComparer.Default.Equals(objectSymbol, classSymbol.BaseType) && !SymbolEqualityComparer.Default.Equals(valueTypeSymbol, classSymbol.BaseType))
                hasBaseType = true;
        }
    ...
    private static (PropertyType Type, string ComplexTypeName, bool IsArray) ParseType(ITypeSymbol typeSymbol, INamedTypeSymbol? inlineArraySymbol)
    {
        switch (typeSymbol)
        {
